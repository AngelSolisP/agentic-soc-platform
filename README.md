...
Token reduction vs naive prompt-stuffing: ~40K → 2-8K (5-20x). See `runbooks/tactical/` for live examples.
...
Three views per case. **Summary** shows alerts, IoCs, and the proposed action awaiting analyst decision. **Pipeline** is the Glass Box, the full structured output of each agent stage, on-demand. **Timeline** is the chronological event log.
